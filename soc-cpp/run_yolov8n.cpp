...
/* --------------------------------------------------------------------------
 * pipeline_main.c에서 사용하는 YOLO 실행 상태. VectorBlox DMA allocator는
 * 개별 free를 지원하지 않으므로 모델과 입출력 버퍼를 한 번만 할당해 재사용한다.
 * -------------------------------------------------------------------------- */
static vbx_cnn_t *pipeline_vbx = NULL;
static model_t *pipeline_model = NULL;
static vbx_cnn_io_ptr_t pipeline_io[MAX_IO_BUFFERS] = {};
static yolov8n_output_t pipeline_outputs[YOLOV8N_NUM_OUTPUTS] = {};
static int pipeline_input_width = 0;
static int pipeline_input_height = 0;
static int pipeline_input_is_hcw = 0;
static int pipeline_output_channels = 0;
static int pipeline_num_classes = 0;
static unsigned pipeline_input_count = 0;
static unsigned pipeline_output_count = 0;
static float pipeline_confidence = 0.30f;
static float pipeline_nms = 0.40f;

extern "C" int yolo_init(const char *model_path,
                          ...
                          float nms_iou_threshold)
{
    ...
    if (!model_path || confidence_threshold < 0.0f ||
        confidence_threshold > 1.0f || nms_iou_threshold < 0.0f ||
        nms_iou_threshold > 1.0f) return -1;
    if (pipeline_model) return 0;

    pipeline_vbx = vbx_cnn_init(NULL);
    if (!pipeline_vbx) return -2;
    pipeline_model = read_model_file(pipeline_vbx, model_path);
    if (!pipeline_model || model_check_sanity(pipeline_model) != 0) return -3;

    pipeline_input_count = model_get_num_inputs(pipeline_model);
    pipeline_output_count = model_get_num_outputs(pipeline_model);
    if (pipeline_input_count != 1 ||
        pipeline_output_count != YOLOV8N_NUM_OUTPUTS) return -4;

    pipeline_output_channels =
        infer_output_channels(pipeline_model, pipeline_output_count);
    pipeline_num_classes = pipeline_output_channels - YOLOV8N_DFL_CHANNELS;
    if (pipeline_num_classes <= 0) return -5;

    shape = model_get_input_shape(pipeline_model, 0);
    dims = model_get_input_dims(pipeline_model, 0);
    if (!shape || (dims != 3 && dims != 4) ||
        (dims == 4 && shape[0] != 1)) return -6;
    if (shape[dims - 3] == 3) {
        pipeline_input_height = shape[dims - 2];
        pipeline_input_width = shape[dims - 1];
    } else if (shape[dims - 2] == 3) {
        pipeline_input_is_hcw = 1;
        pipeline_input_height = shape[dims - 3];
        pipeline_input_width = shape[dims - 1];
    } else return -7;
    if (model_get_input_datatype(pipeline_model, 0) !=
            VBX_CNN_CALC_TYPE_UINT8 ||
        model_get_input_zeropoint(pipeline_model, 0) != 0) return -8;

    pipeline_io[0] = reinterpret_cast<vbx_cnn_io_ptr_t>(
        vbx_allocate_dma_buffer(
            pipeline_vbx, model_get_input_length(pipeline_model, 0), 0));
    if (!pipeline_io[0]) return -9;
    for (unsigned o = 0; o < pipeline_output_count; ++o) {
        const size_t bytes = model_get_output_length(pipeline_model, o) *
                             sizeof(int8_t);
        pipeline_io[pipeline_input_count + o] =
            reinterpret_cast<vbx_cnn_io_ptr_t>(
                vbx_allocate_dma_buffer(pipeline_vbx, bytes, 0));
        if (!pipeline_io[pipeline_input_count + o]) return -10;
    }

#if USE_INTERRUPTS
    {
        uint32_t enable = 1;
        if (write(pipeline_vbx->fd, &enable, sizeof(enable)) < 0) return -11;
    }
#endif
    pipeline_confidence = confidence_threshold;
    pipeline_nms = nms_iou_threshold;
    std::printf("YOLO initialized: input=%dx%d classes=%d conf=%.3f nms=%.3f\n",
                pipeline_input_width, pipeline_input_height,
                pipeline_num_classes, pipeline_confidence, pipeline_nms);
    return 0;
}

//yolo_detect() 
extern "C" int yolo_detect(const uint8_t *pixels,
                            int width,
                            int height,
                            int stride_bytes,
                            int channels,
                            ...
                            int max_detections)
{
    ...
    if (!pipeline_model || !pixels || !detections || max_detections <= 0 ||
        width <= 0 || height <= 0 || channels != 3 ||
        stride_bytes < width * channels) return -1;

    if (stride_bytes != width * channels) {
        packed = static_cast<uint8_t *>(
            std::malloc(static_cast<size_t>(width) * height * channels));
        if (!packed) return -2;
        for (int y = 0; y < height; ++y) {
            std::memcpy(packed + static_cast<size_t>(y) * width * channels,
                        pixels + static_cast<size_t>(y) * stride_bytes,
                        static_cast<size_t>(width) * channels);
        }
        pixels = packed;
    }
    image = preprocess_rgb_planar_image(pixels, width, height,
                                        pipeline_input_width,
                                        pipeline_input_height);
    std::free(packed);
    if (!image) return -3;
    if (pipeline_input_is_hcw) {
        image = repack_chw_to_hcw(image, pipeline_input_width,
                                  pipeline_input_height);
        if (!image) return -4;
    }
    std::memcpy(reinterpret_cast<void *>(pipeline_io[0]), image,
                model_get_input_length(pipeline_model, 0));
    std::free(image);

    status = vbx_cnn_model_start(pipeline_vbx, pipeline_model, pipeline_io);
    if (status >= 0) {
#if USE_INTERRUPTS
        status = vbx_cnn_model_wfi(pipeline_vbx);
#else
        while ((status = vbx_cnn_model_poll(pipeline_vbx)) > 0) {}
#endif
    }
    ...
    for (unsigned o = 0; o < pipeline_output_count; ++o) {
        if (configure_output(
                pipeline_model, static_cast<int>(o),
                reinterpret_cast<const int8_t *>(
                    pipeline_io[pipeline_input_count + o]),
                pipeline_output_channels, &pipeline_outputs[o]) != 0) {
            return -6;
        }
    }

    config.input_width = pipeline_input_width;
    config.input_height = pipeline_input_height;
    config.image_width = width;
    config.image_height = height;
    config.num_classes = pipeline_num_classes;
    config.transpose_xy = pipeline_input_is_hcw ? 1 : 0;
    config.confidence_threshold = pipeline_confidence;
    config.nms_iou_threshold = pipeline_nms;
    return yolov8n_postprocess(pipeline_outputs, &config,
